...
const BOLL_K = 2;         // 标准差倍数
const BOLL_LOOKBACK = 40; // 拉取日K条数
...
// '1.601318' / '0.000001' → 'sh601318' / 'sz000001'
function toTxSymbol(secid) {
  const p = secid.split('.');
  return (p[0] === '1' ? 'sh' : 'sz') + p[1];
}

// ═══ 腾讯行情快照（批量，一次拿全部；GBK 编码）═══
        ...
        try {
          ...
          text.split(';').forEach(function (line) {
            ...
            out[code] = {
              ...
              marketCap: parseFloat(f[45]) || 0,      // 亿元
              time: f[30],
            };
          });
          resolve(out);
...
// ═══ 日K拉取（多源回退：东财 → 腾讯 → 新浪）═══
// 东财 push2his 在部分网络环境会被拦截，故必须保留腾讯回退
// 源1：东方财富（原始首选，前复权 fqt=1）
...
function calcBoll(closes, n, k) {
  ...
  return {
    ...
    upper: ma + k * sd,
    lower: ma - k * sd,
    n: n, k: k, bars: closes.length,
    width: ma > 0 ? (2 * k * sd / ma * 100) : 0,   // 带宽，衡量波动率
    pctB: (k * sd) > 0 ? ((last - (ma - k * sd)) / (2 * k * sd) * 100) : 50, // 0=下轨 50=中轨 100=上轨
  };
}

// ═══ 由布林带推导最优买点区间 ═══
// 下沿 = 布林下轨
// 上沿 = 下轨 + 0.3×(中轨−下轨)（硬上限，防止区间过宽失去指导意义）
//        若 MA5/MA10 落在 [下轨, 该上限] 内，取较高者
function deriveBuyZone(boll, ma5, ma10) {
  if (!boll) return null;
  const lower = boll.lower, mid = boll.ma;
  const span = mid - lower;
...
async function getAllStockData() {
  ...
  for (const s of STOCKS) {
    ...
    let boll = null, buyZone = null, effectiveTarget = s.target;
    try {
      ...
      if (boll) {
        ...
        buyZone = deriveBuyZone(boll, maOf(closes, 5), maOf(closes, 10));
        if (buyZone) effectiveTarget = buyZone.low;
      }
    } catch (e) {
...
http.createServer(async (req, res) => {
  ...
  if (req.url.indexOf('/api/stocks') === 0) {
    const now = Date.now();
